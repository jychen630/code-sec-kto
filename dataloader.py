...
def get_primevul(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str) -> Dataset:
    # split will be either train, valid or test
    assert split in ["train", "valid", "test"], f"split argument can only be 'train', 'valid' or 'test'"
    data = Dataset('primevul')
    data_path = f"/local/arise/junyao/code-sec/data/primevul/mini/mini_primevul_{split}.jsonl"
...
class DataLoader:
    ...
    Always assumes preference pairs. Need extension by downstream developers.
    """
    def __init__(self, 
